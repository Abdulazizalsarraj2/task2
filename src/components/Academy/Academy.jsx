...
export default function Academy() {
  ...
  return (
    <div className="my-32 px-12 mr-8" dir="rtl">
      {/* Dropdown الفلترة بتصميم مخصص */}
      <div className="mb-8 w-full sm:max-w-96" ref={dropdownRef}>
        <div
          onClick={() => setIsOpen(!isOpen)}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-5">
        {filteredPlaylists.map((course) => (
          <Link 
            ...
            className="block"
          >
            <div className="bg-[#FEFEFF] p-1 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 cursor-pointer w-[85%]">
              <img
                src={course.image}
                alt={course.name}
                className="w-full h-40 object-cover rounded-xl"
